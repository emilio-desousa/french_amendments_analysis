...
class TopicWordsFinder(): 
    ...
    def words_per_topic(self):
        ...
        c_tf_idf = self.get_custom_tfidf(df_topic[stg.AMENDEMENT], len(self.df), count_vectorizer)
        top_n_words = self.extract_top_n_words_per_topic(c_tf_idf, count_vectorizer, df_topic, n=20)
    ...
    def extract_topic_sizes(self,df):
        topic_sizes = (df.groupby([stg.TOPIC], sort=True)
                         [stg.AMENDEMENT]
                         .count()
                         .reset_index()
