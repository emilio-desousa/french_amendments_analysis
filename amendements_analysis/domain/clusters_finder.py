...
class clusters_finder():
    ...
    ----------
    Return:
    umap_model: fitted umap model with attribute 'sentence_embeddings'
    kmenoid_model: fitted kmenoid model from umaped 'sentence_embeddings'
    """
    def __init__(self, sentence_embeddings, n_umap = 15, n_clusters = 13):
        ...
        n_clusters = number of topics to split the dataset
        n_umap = number of axis after UMAP size reduction 
        """
    ...
    def models(self):
        ...
        kmenoid_model = KMedoids(n_clusters=self.n_clusters, metric = 'cosine', init = 'random', random_state = 15)
        cluster =  kmenoid_model.fit(umap_embeddings)
        return cluster, umap_model

if "__main__" == __name__ :
    sentence_embeddings = np.load(os.path.join(stg.DATA_DIR, stg.SENTENCE_EMBEDDINGS_FILENAME))
